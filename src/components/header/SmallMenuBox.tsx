...
import australiaFlag from "../../assets/australia-flag.jpeg";
import { Link } from "react-router-dom";
import { useState } from "react";
import SmallShopCard from "./SmallShopCard";
import SmallFeatureCard from "./SmallFeatureCard";
import SmallInspirationCard from "./SmallInspirationCard";
import SmallSupportCard from "./SmallSupportCard";
...
const StyledMenuImageStack = styled(Stack)(() => ({
  alignItems: "center",
...
type HandlerProps = {
  handleShowDrawerMenu: () => void;
  communicateMenuHover: (ishovering: boolean) => void;
};
const SmallMenuBox = ({
  handleShowDrawerMenu,
  communicateMenuHover,
}: HandlerProps) => {
  const [isShowShopCard, setIsShowShopCard] = useState(false);
  const [isShowNewFeatureCard, setIsShowNewFeatureCard] = useState(false);
  const [isShowInspirationCard, setIsShowInspiraionCard] = useState(false);
  const [isShowSupportCard, setIsShowSupportCard] = useState(false);
  ...
  return (
    <>
      {isOpenSearchCard ? (
        ...
        <Box
          onMouseLeave={() => {
            communicateMenuHover(false);
          }}
          sx={{
            bgcolor: "rgb(49,54,57)",
            height: "100%",
            width: "100%",
            color: "white",
            cursor: "pointer",
        ...
        >
          ...
          >
            <IconButton onClick={handleShowDrawerMenu} sx={{ color: "white" }}>
              <Clear />
            </IconButton>
            <Link to="/home">
              <SvgIcon
            ...
            >
              <Box
                component="i"
                sx={{ fontSize: 25 }}
                onClick={() => setIsOpenSearchCard(true)}
              >
                <FiSearch />
              </Box>
              <Box component="i">
          ...
          {isShowShopCard ? (
            <SmallShopCard setIsShowShopCard={setIsShowShopCard} />
          ) : isShowNewFeatureCard ? (
            <SmallFeatureCard
              setIsShowNewFeatureCard={setIsShowNewFeatureCard}
            />
          ) : isShowInspirationCard ? (
            <SmallInspirationCard
              setIsShowInspiraionCard={setIsShowInspiraionCard}
            />
          ) : isShowSupportCard ? (
            <SmallSupportCard setIsShowSupportCard={setIsShowSupportCard} />
          ) : (
            <Stack>
              <StyledMenuStack
                onClick={() => {
                  setIsShowShopCard(!isShowShopCard);
                }}
              >
              ...
              <StyledMenuStack
                onClick={() => setIsShowNewFeatureCard(!isShowNewFeatureCard)}
              >
                <Typography fontSize="20px" fontWeight="bold">
                  New & Featured
                </Typography>
              ...
              <StyledMenuStack
                onClick={() => setIsShowInspiraionCard(!isShowInspirationCard)}
              >
                <Typography fontSize="20px" fontWeight="bold">
                  Inspiration
                </Typography>
              ...
              <StyledMenuStack
                onClick={() => setIsShowSupportCard(!isShowSupportCard)}
              >
                <Typography fontSize="20px" fontWeight="bold">
                  Support
                </Typography>
              ...
              />
              <StyledMenuImageStack>
                <PersonOutline />
                <Typography fontSize="20px" fontWeight="bold" marginLeft="10px">
                  Sign In
                </Typography>
              </StyledMenuImageStack>
              <Divider
